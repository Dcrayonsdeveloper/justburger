/**
 * JustBurgers Main App Service Worker
 *
 * Strategy:
...
self.addEventListener('push', event => {
    ...
    try {
        ...
        data = { title: 'JustBurgers', body: event.data.text() };
    }

    const options = {
        body: data.body || '',
        icon: data.icon || '/images/justburgers-logo.png',
        badge: '/images/justburgers-logo.png',
        image: data.image || undefined,
        data: { url: data.url || '/' },
        actions: data.actions || [],
        vibrate: [100, 50, 100],
        tag: data.tag || 'justburgers-notification',
        renotify: true,
    };

    event.waitUntil(
        self.registration.showNotification(data.title || 'JustBurgers', options)
    );
});
