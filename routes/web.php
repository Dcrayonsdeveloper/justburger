...
Route::get('/sitemap-blog.xml', fn () => redirect('/sitemap.xml', 301));
...
// Redirects: old /products/ and /product/ URLs → /menu/ (301 SEO-safe)
Route::get('/products/{slug?}', function (string $slug = null) {
    return $slug
        ? redirect()->route('products.show', $slug, 301)
        : redirect()->route('products.index', [], 301);
});
...
// Product Toppings (AJAX)
Route::get('/api/product/{product:id}/toppings', App\Http\Controllers\Api\ProductToppingController::class)->name('product.toppings');
...
Route::get('/blog', fn () => redirect('/', 301))->name('blog');
Route::get('/blog/{slug}', fn () => redirect('/', 301))->name('blog.show');
Route::get('/careers', [App\Http\Controllers\PageController::class, 'careers'])->name('careers');
Route::get('/rewards', [App\Http\Controllers\PageController::class, 'rewards'])->name('rewards');
Route::get('/help', [App\Http\Controllers\PageController::class, 'help'])->name('help');
Route::get('/returns-policy', fn () => redirect('/', 301))->name('returns');
Route::get('/shipping', [App\Http\Controllers\PageController::class, 'shipping'])->name('shipping');
Route::get('/size-guide', fn () => redirect('/', 301))->name('size-guide');
...
Route::fallback(fn () => redirect('/', 302));
