...
class JustBurgersBoostSeeder extends Seeder
{
    public function run(): void
    {
        // ── 1. Create/assign "JustBurgers" brand to all products ──
        $brand = Brand::firstOrCreate(
            ['slug' => 'justburgers'],
            [
                'name' => config('app.name', 'JustBurgers'),
                'description' => config('app.name', 'JustBurgers') . ' - Quality products for everyday life.',
                'is_active' => true,
                'is_featured' => true,
                'position' => 0,
            ]
        ...
        $this->command->info("Brand '" . config('app.name', 'JustBurgers') . "' (ID: {$brand->id}) assigned to {$updated} products.");
        ...
        $reviewBodies = [
            5 => [
                ...
                'Superb product! My family loves it. Will definitely buy more from JustBurgers.',
                'Outstanding quality and fast delivery. Highly recommend to everyone!',
                'Best product in this price range. JustBurgers never disappoints!',
            ],
            4 => [
