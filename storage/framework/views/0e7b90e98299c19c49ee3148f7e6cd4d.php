                ...
                <div class="px-5 py-4" style="min-height:120px;max-height:400px;overflow-y:auto">
                    <template x-if="!query && !loading">
                        <div class="flex flex-col items-center py-6 text-center">
                            ...
                            <p class="text-sm text-neutral-500">Find anything in <?php echo e(\App\Models\Setting::get('site_name', config('app.name'))); ?></p>
                        </div>
                    </template>
        ...
        <div class="relative" x-data="{ open: false }">
            ...
            <div x-cloak x-show="open" x-transition @click.away="open = false" class="absolute right-0 mt-2 w-80 bg-white border border-neutral-200 rounded-xl shadow-xl z-50">
                <div class="px-4 py-3 flex items-center justify-between" style="border-bottom:1px solid #eee">
                    ...
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($unreadCount > 0): ?>
                    <span class="text-xs font-medium" style="color:#2c6ecb"><?php echo e($unreadCount); ?> new</span>
                    <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                </div>
                <div class="max-h-80 overflow-y-auto">
                    ...
                    <a href="<?php echo e(route('admin.notifications.read', $notification)); ?>" class="block px-4 py-3 hover:bg-neutral-50" style="border-bottom:1px solid #f5f5f5">
                        <p class="text-sm font-medium text-neutral-900"><?php echo e($notification->title); ?></p>
                        <p class="text-xs text-neutral-500 mt-0.5 truncate"><?php echo e($notification->content); ?></p>
                        <p class="text-xs text-neutral-400 mt-1"><?php echo e($notification->created_at->diffForHumans()); ?></p>
                    </a>
                ...
                <div class="px-4 py-2.5" style="border-top:1px solid #eee">
                    <a href="<?php echo e(route('admin.notifications')); ?>" class="text-xs font-medium" style="color:#2c6ecb">View all</a>
                </div>
            </div>
        </div>
        ...
        <div class="relative" x-data="{ open: false }">
            <button @click="open = !open" class="flex items-center gap-1.5" aria-label="User menu">
                <div class="w-7 h-7 rounded-full flex items-center justify-center text-xs font-semibold text-white" style="background:#36a854">
                    <?php echo e(strtoupper(substr(auth('admin')->user()->full_name ?? auth('admin')->user()->email ?? 'A', 0, 1))); ?>
            ...
            <div x-cloak x-show="open" x-transition @click.away="open = false" class="absolute right-0 mt-2 w-48 bg-white border border-neutral-200 rounded-xl shadow-xl z-50">
                <div class="px-4 py-3" style="border-bottom:1px solid #eee">
                    <div class="text-sm font-medium text-neutral-900"><?php echo e(auth('admin')->user()->full_name ?? 'Admin'); ?></div>
                    <div class="text-xs text-neutral-500"><?php echo e(auth('admin')->user()->email); ?></div>
                </div>
                <a href="<?php echo e(route('admin.profile')); ?>" class="block px-4 py-2 text-sm text-neutral-700 hover:bg-neutral-50">Profile</a>
                <a href="<?php echo e(url('/')); ?>" target="_blank" class="block px-4 py-2 text-sm text-neutral-700 hover:bg-neutral-50">View Store</a>
...
<?php /**PATH /home/u322703740/domains/justburger.dcrayons.app/resources/views/admin/partials/header.blade.php ENDPATH**/ ?>