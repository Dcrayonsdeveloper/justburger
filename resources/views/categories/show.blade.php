<x-layouts.app>
    <x-slot name="title">{{ $category->name }} - {{ \App\Models\Setting::get('site_name', config('app.name')) }}</x-slot>

    @push('meta')
        @php
            $catDesc = $category->meta_description ?? $category->description ?? "Shop {$category->name} at " . \App\Models\Setting::get('site_name', config('app.name')) . ". Browse {$products->total()} products with great prices.";
        @endphp
        <meta name="description" content="{{ Str::limit(strip_tags($catDesc), 160) }}">
        <link rel="canonical" href="{{ route('category.show', $category->slug) }}">
        <meta property="og:title" content="{{ $category->name }} - {{ \App\Models\Setting::get('site_name', config('app.name')) }}">
        ...
        <meta name="twitter:title" content="{{ $category->name }} - {{ \App\Models\Setting::get('site_name', config('app.name')) }}">
