                    ...
                    @foreach($filterSubcategories as $sub)
                        <label class="flex items-center gap-2.5 cursor-pointer group py-0.5">
                            <input type="checkbox" name="subcategory[]" value="{{ $sub->slug }}"
                                   ...
                                   class="w-3.5 h-3.5 rounded border-neutral-300 text-[#205258] focus:ring-[#205258] focus:ring-offset-0">
            ...
            <div class="flex items-center gap-2 pt-1 pb-2">
                <div class="relative flex-1">
                    ...
                    <input type="number" name="min_price" value="{{ request('min_price') }}"
                           placeholder="Min" class="w-full pl-6 pr-2 py-2 text-sm border border-neutral-200 rounded-lg focus:outline-none focus:border-[#205258] bg-neutral-50">
                </div>
                <span class="text-neutral-300 text-sm">—</span>
                <div class="relative flex-1">
                    <span class="absolute left-2.5 top-1/2 -translate-y-1/2 text-xs text-neutral-600">£</span>
                    <input type="number" name="max_price" value="{{ request('max_price') }}"
                           placeholder="Max" class="w-full pl-6 pr-2 py-2 text-sm border border-neutral-200 rounded-lg focus:outline-none focus:border-[#205258] bg-neutral-50">
                </div>
            </div>
        </div>
    </div>
