       ...
       onmouseleave="this.style.transform='';this.style.boxShadow='';this.style.borderColor='';">
        ...
        <div style="padding:.9rem 1rem 1rem; display:flex; flex-direction:column; flex:1;">
            ...
            <div style="display:flex;align-items:center;gap:.35rem;margin-bottom:.6rem;">
                <div style="display:flex;gap:1px;">
                    @for($i = 1; $i <= 5; $i++)
                        <svg style="width:12px;height:12px;fill:{{ $i <= round($rating) ? '#D4A017' : 'rgba(255,255,255,.18)' }};" viewBox="0 0 20 20">
                            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                        </svg>
                    @endfor
                </div>
                <span style="font-size:.7rem;color:rgba(255,255,255,.32);">({{ $reviewCount }})</span>
            </div>
            @endif
            ...
            <div style="margin-top:auto;padding-top:.6rem;border-top:1px solid rgba(255,255,255,.06);">
                <span style="font-size:1.1rem;font-weight:800;color:#D4A017;display:block;margin-bottom:.55rem;">@price($product->price)</span>
                <button @click="$store.toppingsModal.open({{ $product->id }}, 1)"
                        class="btn-add-order">
                    Add to Order
                </button>
