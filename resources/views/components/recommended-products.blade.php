...
<div x-data="recommendedProducts()" x-init="load()" x-show="products.length > 0" x-cloak class="mt-8">
        ...
        <template x-for="product in products" :key="product.id">
            <a :href="'/products/' + product.slug" class="group block bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow p-3">
                ...
                <div x-show="product.rating > 0" class="flex items-center gap-1 mt-1">
                    ...
                    <span x-text="product.rating.toFixed(1)" class="text-xs text-gray-600"></span>
                </div>
            </a>
        </template>
    </div>
...
function recommendedProducts() {
    return {
        ...
        async load() {
            try {
                const res = await fetch('/recommendations/personalized');
