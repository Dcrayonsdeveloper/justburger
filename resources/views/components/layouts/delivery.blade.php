...
<head>
    ...
    <title>{{ $title ?? 'Dashboard' }} - Delivery Panel | {{ \App\Models\Setting::get('site_name', config('app.name')) }}</title>
...
<body class="font-sans antialiased bg-neutral-100 layout-admin" x-data="{ sidebarOpen: false }">
    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
            ...
            <!-- Navigation -->
            <nav class="p-4 space-y-1 overflow-y-auto h-[calc(100vh-4rem)] scrollbar-dark text-[16px]">
                <!-- Dashboard -->
                <a href="{{ route('delivery.dashboard') }}"
