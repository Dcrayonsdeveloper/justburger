<x-layouts.app>
    <x-slot name="title">Reels & Videos - {{ \App\Models\Setting::get('site_name', config('app.name')) }}</x-slot>

    @push('meta')
        <meta name="description" content="Watch the latest reels and videos from {{ \App\Models\Setting::get('site_name', config('app.name')) }}. See our products in action!">
        <meta property="og:title" content="Reels & Videos - {{ \App\Models\Setting::get('site_name', config('app.name')) }}">
        <meta property="og:description" content="Watch the latest reels and videos from {{ \App\Models\Setting::get('site_name', config('app.name')) }}.">
        ...
        <?php
            $fallbackThumb = asset('images/banner.png');
            $reelsSchema = [
                '@context' => 'https://schema.org',
                '@type' => 'CollectionPage',
                'name' => 'Reels & Videos - ' . \App\Models\Setting::get('site_name', config('app.name')),
                'description' => 'Watch the latest product videos and reels from ' . \App\Models\Setting::get('site_name', config('app.name')) . '. Shop trending products now.',
                ...
                'mainEntity' => [
                    ...
                    'itemListElement' => collect($reels)->map(function ($r, $i) use ($fallbackThumb) {
                        ...
                        $caption = $r['caption'] ?: \App\Models\Setting::get('site_name', config('app.name')) . ' - Product Video';
                        ...
                        $item = [
                            ...
                            'description' => Str::limit($r['caption'] ?: 'Watch this product video from ' . \App\Models\Setting::get('site_name', config('app.name')), 300),
                            'thumbnailUrl' => $thumb,
                            'uploadDate' => $date,
