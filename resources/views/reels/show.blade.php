<x-layouts.app>
    <x-slot name="title">{{ Str::limit($reel['caption'], 60) ?: 'Watch Reel' }} - {{ \App\Models\Setting::get('site_name', config('app.name')) }}</x-slot>

    @push('meta')
        <meta name="description" content="{{ Str::limit($reel['caption'], 160) }}">
        <meta property="og:title" content="{{ Str::limit($reel['caption'], 60) ?: 'Watch Reel' }} - {{ \App\Models\Setting::get('site_name', config('app.name')) }}">
        ...
        <?php
            ...
            $reelCaption = $reel['caption'] ?: \App\Models\Setting::get('site_name', config('app.name')) . ' - Product Video';
            $reelDesc = $reel['caption'] ?: 'Watch this product video from ' . \App\Models\Setting::get('site_name', config('app.name')) . '. Shop the latest trending products.';
            $reelThumb = $reel['thumbnail_url'] ?: asset('images/banner.png');
