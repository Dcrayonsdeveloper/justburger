<x-layouts.app>
    <x-slot name="title">About Us - {{ \App\Models\Setting::get('site_name', config('app.name')) }}</x-slot>

    @push('meta')
        <meta name="description" content="{{ \App\Models\Setting::get('site_name', 'Just Burgers Plus') }} — charbroiled burgers in Bedfont since 1989. {{ \App\Models\Setting::get('site_address', '525 Staines Road, Bedfont, Middx. TW14 8BP') }}. We have no other branch.">
        <link rel="canonical" href="{{ url('/about') }}">
        <meta property="og:title" content="Our Story - {{ \App\Models\Setting::get('site_name', config('app.name')) }}">
        <meta property="og:description" content="{{ \App\Models\Setting::get('site_name', 'Just Burgers Plus') }} — charbroiled burgers in Bedfont since 1989. We have no other branch.">
        <meta property="og:type" content="website">
        <meta property="og:url" content="{{ url('/about') }}">
        <meta name="twitter:card" content="summary">
        <meta name="twitter:title" content="About Us - {{ \App\Models\Setting::get('site_name', config('app.name')) }}">
        <meta name="twitter:description" content="Learn about {{ \App\Models\Setting::get('site_name', config('app.name')) }} - your go-to destination for fresh, handcrafted burgers and bold flavors.">
    @endpush
    ...
    <section class="relative overflow-hidden bg-[#111111] min-h-[500px] lg:min-h-[600px] flex items-center">
        <div class="absolute inset-0 bg-cover bg-center opacity-40" style="background-image: url('{{ asset('images/banners/banner1.webp') }}');"></div>
        ...
        <div class="container mx-auto px-6 lg:px-8 relative z-10 py-20 lg:py-28">
            <div class="max-w-2xl">
                ...
                <p class="text-lg sm:text-xl text-white/80 max-w-lg leading-relaxed">
                    {{ \App\Models\Setting::get('about_hero_text', 'Charbroiled burgers in Feltham since 1989. We have no other branch.') }}
                </p>
            </div>
        </div>
            ...
            <div class="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
                ...
                <div class="relative">
                    <div class="aspect-[4/3] rounded-2xl overflow-hidden">
                        <img src="{{ asset('images/banners/our-story.jpg') }}" alt="Our Story" class="w-full h-full object-cover" loading="lazy">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="space-y-4 text-base text-white/60 leading-relaxed">
                        <p>
                            {{ \App\Models\Setting::get('about_story_p1', 'Just Burgers Plus has been a Feltham favourite since 1989. What started as a small charbroiled burger shop on Staines Road has grown into one of the most trusted takeaways in the area — serving fresh, flame-grilled burgers to generations of loyal customers.') }}
                        </p>
                        <p>
                            {{ \App\Models\Setting::get('about_story_p2', 'For over 35 years, our commitment has never changed: quality ingredients, proper cooking methods, and generous portions at fair prices. Every burger is charbroiled to order — never pre-made, never microwaved.') }}
                        </p>
                        <p>
                            {{ \App\Models\Setting::get('about_story_p3', 'From our classic Cheese Burger to the loaded Gangsta Burger, from crispy Chicken Strips to creamy Milkshakes — everything on our menu is made with care. We have no other branch, so all our attention goes into this one shop, making sure every customer leaves happy.') }}
                        </p>
                    </div>
                </div>
        ...
        <div class="container mx-auto px-6 lg:px-8">
            <div class="max-w-4xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                <div>
                    ...
                    <a href="{{ route('products.index') }}" class="inline-block bg-[#C8102E] text-white px-8 py-3 font-bold text-sm hover:bg-[#a00d24] transition-colors">
                        View Full Menu
                    </a>
                </div>
                <div class="relative">
                    <div class="aspect-[4/3] rounded-2xl overflow-hidden bg-[#111111]">
                        <img src="{{ asset('images/banners/banner2.webp') }}" alt="Our Menu" class="w-full h-full object-cover" loading="lazy">
                    </div>
                </div>
            </div>
        </div>
    ...
    <section class="relative overflow-hidden bg-[#C8102E]">
        <div class="absolute inset-0 bg-cover bg-center opacity-20" style="background-image: url('{{ asset('images/banners/banner2.webp') }}');"></div>
        <div class="container mx-auto px-6 lg:px-8 relative z-10 py-20 lg:py-28">
            <div class="max-w-2xl mx-auto text-center">
                ...
                <div class="flex flex-wrap items-center justify-center gap-4">
                    <a href="tel:{{ \App\Models\Setting::get('site_phone', '+4420889 05008') }}" class="inline-flex items-center gap-2 bg-white text-[#C8102E] rounded px-8 py-3 font-bold text-sm hover:bg-white/90 transition-all duration-300">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/></svg>
                        Call to Order
                    </a>
                    <a href="https://wa.me/{{ \App\Models\Setting::get('whatsapp_number', '447368429896') }}" target="_blank" rel="noopener" class="inline-flex items-center gap-2 border-2 border-white text-white rounded px-8 py-2.5 font-bold text-sm hover:bg-white hover:text-[#C8102E] transition-all duration-300">
