    ...
    <div class="max-w-3xl" x-data="categoryForm()">
        <form action="{{ route('admin.categories.update', $category) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="card overflow-hidden">
                ...
                <div class="p-6 space-y-5">
                    ...
                    <!-- Thumbnail -->
                    <div x-data="{ preview: null, removing: false }">
                        ...
                        <div class="flex items-start gap-4">
                            <div class="w-20 h-20 rounded-xl bg-neutral-100 border-2 border-dashed border-neutral-300 flex items-center justify-center overflow-hidden shrink-0">
                                ...
                                <template x-if="!preview && !removing">
                                    @if($category->image_url)
                                        <img src="{{ asset(ltrim($category->image_url, '/')) }}" class="w-full h-full object-cover rounded-lg">
                                    @else
                            ...
                            <div class="flex-1">
                                ...
                                <p class="form-help">JPG, PNG or WebP. Max 2MB. Leave empty to keep current image.</p>
                                @error('image')
                                    <p class="form-error">{{ $message }}</p>
                                @enderror
                                @if($category->image_url)
                                    <label class="flex items-center gap-2 mt-2 cursor-pointer">
                                        <input type="checkbox" name="remove_image" value="1" class="form-checkbox"
                                               x-model="removing" @change="if(removing) { preview = null; $refs.imageInput?.value && ($refs.imageInput.value = '') }">
                                        <span class="text-sm text-error-600">Remove current image</span>
                                    </label>
                                @endif
                            </div>
                        </div>
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="3"
                                  class="form-input w-full @error('description') form-input-error @enderror">{{ old('description', $category->description) }}</textarea>
                        @error('description')
                            <p class="form-error">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        <label class="flex items-center gap-2.5 cursor-pointer">
                            <input type="checkbox" name="exclude_from_bestsellers" value="1" {{ old('exclude_from_bestsellers', $category->exclude_from_bestsellers) ? 'checked' : '' }}
                                   class="form-checkbox">
                ...
                <div class="p-6 space-y-5">
                    ...
                    <div>
                        ...
                        <textarea name="meta_description" id="meta_description" rows="2"
                                  class="form-input w-full @error('meta_description') form-input-error @enderror">{{ old('meta_description', $category->meta_description) }}</textarea>
        ...
        <div class="mt-8 card overflow-hidden border-error-200">
            ...
            <div class="p-6 flex items-center justify-between">
                <div>
                    <p class="text-sm font-medium text-neutral-900">Delete this category</p>
