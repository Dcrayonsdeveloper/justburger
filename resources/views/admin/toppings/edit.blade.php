<x-layouts.admin>
    <x-slot name="title">Edit Topping — {{ $topping->name }}</x-slot>
    ...
    <div class="max-w-lg">
        <form action="{{ route('admin.toppings.update', $topping) }}" method="POST" class="card p-6 space-y-5">
            @csrf @method('PUT')

            <div>
                <label for="name" class="block text-sm font-medium text-neutral-700 mb-1">Name <span class="text-red-500">*</span></label>
                <input type="text" name="name" id="name" value="{{ old('name', $topping->name) }}" required
            ...
            <div>
                ...
                <input type="number" name="price" id="price" value="{{ old('price', $topping->price) }}" step="0.01" min="0"
                       class="w-full px-3 py-2 border border-neutral-300 rounded-lg text-sm focus:ring-2 focus:ring-primary-500 focus:border-primary-500">
            ...
            <div>
                ...
                <select name="group" id="group" class="w-full px-3 py-2 border border-neutral-300 rounded-lg text-sm focus:ring-2 focus:ring-primary-500 focus:border-primary-500">
                    <option value="veggies" {{ old('group', $topping->group) === 'veggies' ? 'selected' : '' }}>Veggies</option>
                    <option value="cheese" {{ old('group', $topping->group) === 'cheese' ? 'selected' : '' }}>Cheese</option>
                    <option value="sauce" {{ old('group', $topping->group) === 'sauce' ? 'selected' : '' }}>Sauce</option>
                    <option value="extras" {{ old('group', $topping->group) === 'extras' ? 'selected' : '' }}>Extras</option>
                </select>
            ...
            <div>
                ...
                <input type="number" name="position" id="position" value="{{ old('position', $topping->position) }}" min="0"
                       class="w-full px-3 py-2 border border-neutral-300 rounded-lg text-sm focus:ring-2 focus:ring-primary-500 focus:border-primary-500">
            ...
            <div class="flex items-center gap-2">
                ...
                <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $topping->is_active) ? 'checked' : '' }}
