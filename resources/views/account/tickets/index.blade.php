            ...
            <div class="flex-1 min-w-0">
                ...
                <div class="flex gap-2 mb-4">
                    <a href="{{ route('account.tickets.index') }}"
                       class="px-3 py-1.5 rounded-full text-xs font-medium {{ !request('status') ? 'bg-[#205258]/10 text-[#1b454a]' : 'bg-neutral-100 text-neutral-600 hover:bg-neutral-200' }} transition-colors">
                        All
                    </a>
                    <a href="{{ route('account.tickets.index', ['status' => 'open']) }}"
                       class="px-3 py-1.5 rounded-full text-xs font-medium {{ request('status') === 'open' ? 'bg-[#205258]/10 text-[#1b454a]' : 'bg-neutral-100 text-neutral-600 hover:bg-neutral-200' }} transition-colors">
                        Open
                    </a>
                    <a href="{{ route('account.tickets.index', ['status' => 'answered']) }}"
                       class="px-3 py-1.5 rounded-full text-xs font-medium {{ request('status') === 'answered' ? 'bg-[#205258]/10 text-[#1b454a]' : 'bg-neutral-100 text-neutral-600 hover:bg-neutral-200' }} transition-colors">
                        Answered
                    </a>
                    <a href="{{ route('account.tickets.index', ['status' => 'closed']) }}"
                       class="px-3 py-1.5 rounded-full text-xs font-medium {{ request('status') === 'closed' ? 'bg-[#205258]/10 text-[#1b454a]' : 'bg-neutral-100 text-neutral-600 hover:bg-neutral-200' }} transition-colors">
                        Closed
                    </a>
                </div>
                ...
                <div class="space-y-3">
                    @forelse($tickets as $ticket)
                        <a href="{{ route('account.tickets.show', $ticket) }}"
                           class="block bg-white border border-neutral-100 rounded-xl p-4 hover:border-[#205258]/30 hover:shadow-sm transition-all">
                    ...
                    @empty
                        <div class="bg-white border border-neutral-100 rounded-xl p-8 text-center">
                            ...
                            <a href="{{ route('account.tickets.create') }}" class="text-sm text-[#205258] font-medium hover:underline">Raise your first ticket</a>
                        </div>
                    @endforelse
                </div>
