        ...
        <div class="flex flex-col lg:flex-row gap-6">
            ...
            <div class="flex-1 min-w-0">
                ...
                <div class="bg-white border border-neutral-100 rounded-xl mb-4">
                    ...
                    <div class="p-5">
                        <div class="flex items-start gap-3">
                            <div class="w-8 h-8 bg-[#205258]/10 rounded-full flex items-center justify-center shrink-0">
                                <span class="text-xs font-semibold text-[#205258]">{{ strtoupper(substr(auth()->user()->first_name, 0, 1)) }}</span>
                            </div>
                            <div class="flex-1 min-w-0">
                ...
                @foreach($ticket->replies as $reply)
                    <div class="bg-white border border-neutral-100 rounded-xl mb-4 {{ $reply->is_admin ? 'border-l-4 border-l-[#205258]' : '' }}">
                        <div class="p-5">
                            <div class="flex items-start gap-3">
                                <div class="w-8 h-8 rounded-full flex items-center justify-center shrink-0 {{ $reply->is_admin ? 'bg-[#F8931D]' : 'bg-[#205258]/10' }}">
                                    <span class="text-xs font-semibold {{ $reply->is_admin ? 'text-white' : 'text-[#205258]' }}">
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-center gap-2 mb-1">
                                        ...
                                        @if($reply->is_admin)
                                            <span class="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-[#205258]/10 text-[#1b454a]">Staff</span>
                                        @endif
