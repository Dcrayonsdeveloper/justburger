                        ...
                        <form action="{{ route('account.returns.store') }}" method="POST"
                                    ...
                                    <div class="grid gap-2">
                                        @foreach($orders as $order)
                                            <label @click="selectedOrder = '{{ $order->id }}'; selectedItems = []"
                                                   :class="selectedOrder == '{{ $order->id }}' ? 'border-[#205258]/50 bg-[#205258]/5 ring-1 ring-[#205258]/30' : 'border-neutral-200 hover:border-neutral-300 bg-white'"
                                                ...
                                                <div class="flex items-center gap-3">
                                                    <div :class="selectedOrder == '{{ $order->id }}' ? 'border-[#205258] bg-[#F8931D]' : 'border-neutral-300'"
                                ...
                                <div class="p-5">
                                    <div class="grid grid-cols-2 gap-3">
                                        <label @click="type = 'return'"
                                               :class="type === 'return' ? 'border-[#205258]/50 bg-[#205258]/5 ring-1 ring-[#205258]/30' : 'border-neutral-200 hover:border-neutral-300'"
                                            ...
                                            <div class="w-9 h-9 rounded-lg flex items-center justify-center shrink-0"
                                                 :class="type === 'return' ? 'bg-[#205258]/10 text-[#205258]' : 'bg-neutral-100 text-neutral-600'">
                                        ...
                                        <label @click="type = 'exchange'"
                                               :class="type === 'exchange' ? 'border-[#205258]/50 bg-[#205258]/5 ring-1 ring-[#205258]/30' : 'border-neutral-200 hover:border-neutral-300'"
                                               class="flex items-center gap-3 p-3.5 rounded-lg border cursor-pointer transition-all">
                                            <input type="radio" name="type" value="exchange" x-model="type" class="sr-only">
                                            <div class="w-9 h-9 rounded-lg flex items-center justify-center shrink-0"
                                                 :class="type === 'exchange' ? 'bg-[#205258]/10 text-[#205258]' : 'bg-neutral-100 text-neutral-600'">
                            ...
                            <div class="bg-white rounded-xl border border-neutral-200 overflow-hidden">
                                ...
                                <div class="p-5 space-y-3">
                                    <select name="reason" required class="w-full rounded-lg border border-neutral-200 text-sm px-3 py-2.5 focus:border-[#205258]/50 focus:ring focus:ring-[#205258]/15 focus:ring-opacity-50">
                                    ...
                                    <textarea name="description" rows="2" placeholder="Additional details (optional)"
                                              class="w-full rounded-lg border border-neutral-200 text-sm px-3 py-2.5 focus:border-[#205258]/50 focus:ring focus:ring-[#205258]/15 focus:ring-opacity-50 resize-none">{{ old('description') }}</textarea>
                                    @error('description')
                                        <p class="text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                </div>
                                ...
                                <div class="p-5">
                                    ...
                                    <div class="space-y-2">
                                        <template x-for="(item, idx) in currentOrder?.items || []" :key="item.id">
                                            <div :class="isSelected(item.id) ? 'border-[#205258]/50 bg-[#205258]/5/30 ring-1 ring-[#205258]/30' : 'border-neutral-200'"
                                                ...
                                                <div @click="toggleItem(item.id)" class="flex items-center gap-3 p-3.5 cursor-pointer">
                                                    <div :class="isSelected(item.id) ? 'bg-[#F8931D] border-[#205258]' : 'border-neutral-300'"
                                                    ...
                                                    <div class="text-right shrink-0">
                                                        <p class="text-sm font-semibold text-neutral-900" x-text="'£' + parseFloat(item.price).toFixed(2)"></p>
                                                    ...
                                                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                                                        ...
                                                        <div>
                                                            ...
                                                            <input type="number"
                                                                   ...
                                                                   class="w-full rounded-lg border border-neutral-200 text-sm px-3 py-2 focus:border-[#205258]/50 focus:ring focus:ring-[#205258]/15 focus:ring-opacity-50">
                                                        </div>
