                    ...
                    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-[16px] text-neutral-600">Placed on {{ $order->created_at->format('d M Y, h:i A') }}</p>
                            </div>
                        </div>
                        @php
                            $statusColors = [
                                    ...
                                    @foreach($order->items as $item)
                                        <div class="flex gap-3.5 p-4">
                                            ...
                                            <div class="flex-1 min-w-0">
                                                <p class="text-[16px] font-semibold text-neutral-900">{{ $item->product_name }}</p>
                                                ...
                                                <div class="flex items-center justify-between mt-2">
                                                    ...
                                                    <span class="text-[16px] font-semibold text-neutral-900">@price($item->total)</span>
                                                </div>
                                            </div>
                                        </div>
                                    @endforeach
                            ...
                            @if($order->notes)
                                <div class="bg-white rounded-xl border border-neutral-100 p-4">
                                    <div class="flex items-center gap-2 mb-2">
                                        ...
                                        <h3 class="text-[16px] font-semibold text-neutral-900">Order Notes</h3>
                                    </div>
                                    <p class="text-[16px] text-neutral-600">{{ $order->notes }}</p>
                                </div>
                            @endif
                        </div>
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="bg-white rounded-xl border border-neutral-100 p-4">
                                ...
                                <dl class="space-y-2.5">
                                    ...
                                    @if($order->shipped_at)
                                        <div class="flex justify-between text-[16px]">
                                            <dt class="text-neutral-600">Out for Delivery</dt>
                            ...
                            <div class="bg-white rounded-xl border border-neutral-100 p-4 space-y-2.5">
                                @if(!in_array($order->status, ['cancelled', 'returned']))
                                    <a href="{{ route('account.orders.track', $order) }}"
                                       class="flex items-center justify-center gap-2 w-full px-4 py-2.5 bg-primary-600 text-white text-[16px] font-semibold rounded-lg hover:bg-primary-700 transition-colors">
                                ...
                                @if($order->canBeCancelled())
                                    <form action="{{ route('account.orders.cancel', $order) }}" method="POST"
                                        ...
                                        @csrf
                                        <button type="submit"
                                                class="flex items-center justify-center gap-2 w-full px-4 py-2.5 border border-red-200 text-red-600 text-[16px] font-medium rounded-lg hover:bg-red-50 hover:border-red-300 transition-all">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <div class="relative">
                                        @foreach($order->statusHistory as $index => $history)
                                            <div class="flex gap-3 {{ !$loop->last ? 'pb-4' : '' }} relative">
                                                ...
                                                <div class="flex-1 min-w-0">
                                                    <p class="text-[16px] font-medium text-neutral-800">{{ str_replace('_', ' ', ucfirst($history->status)) }}</p>
                                                    @if($history->comment)
                            ...
                            <div class="bg-white rounded-xl border border-neutral-100 p-4">
                                <div class="flex items-center gap-2 mb-2">
                                    ...
                                    <h3 class="text-[16px] font-semibold text-neutral-900">Need Help?</h3>
                                </div>
