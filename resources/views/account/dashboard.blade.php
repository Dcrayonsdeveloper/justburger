<x-layouts.app>
    <x-slot name="title">My Account - {{ \App\Models\Setting::get('site_name', config('app.name')) }}</x-slot>
            ...
            <div class="flex-1">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 mb-8">
                    <a href="{{ route('account.orders.index') }}" class="bg-white border border-neutral-100 rounded-xl p-4 text-center hover:border-primary-300 hover:shadow-sm transition-all group">
                        ...
                        <span class="text-[16px] font-medium text-neutral-700">My Orders</span>
                    </a>
                    <a href="{{ route('wishlist') }}" class="bg-white border border-neutral-100 rounded-xl p-4 text-center hover:border-primary-300 hover:shadow-sm transition-all group">
                        ...
                        <span class="text-[16px] font-medium text-neutral-700">Wishlist ({{ $wishlistCount }})</span>
                    </a>
                    {{-- No Addresses tile: orders are collected, so there is no address to keep. --}}
                    <a href="{{ route('account.profile') }}" class="bg-white border border-neutral-100 rounded-xl p-4 text-center hover:border-primary-300 hover:shadow-sm transition-all group">
                        ...
                        <span class="text-[16px] font-medium text-neutral-700">Settings</span>
                    </a>
                </div>
                ...
                <div class="bg-white border border-neutral-100 rounded-xl overflow-hidden">
                    <div class="px-5 py-4 border-b border-neutral-100 flex items-center justify-between">
                        ...
                        <a href="{{ route('account.orders.index') }}" class="text-[16px] text-primary-600 hover:text-primary-700 font-medium">View All</a>
                    </div>
                            ...
                            @foreach($recentOrders as $order)
                                <div class="px-5 py-4 flex items-center justify-between gap-4">
                                    <div class="flex items-center gap-3.5">
                                        ...
                                        <div>
                                            <a href="{{ route('account.orders.show', $order) }}" class="text-[16px] font-semibold text-neutral-900 hover:text-primary-600">
                                    ...
                                    <div class="text-right shrink-0">
                                        <div class="text-[16px] font-semibold text-neutral-900">@price($order->total)</div>
                    ...
                    @else
                        <div class="p-8 text-center">
                            ...
                            <a href="{{ route('products.index') }}" class="inline-flex items-center px-3 py-1.5 text-sm font-semibold text-white bg-primary-600 hover:bg-primary-700 rounded-lg transition-colors">
                                Start Shopping
                            </a>
                        </div>
                    @endif
