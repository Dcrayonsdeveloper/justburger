                ...
                <div class="flex-1">
                    ...
                    <div class="flex items-center justify-between mb-6">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-[16px] text-neutral-600">{{ $addresses->count() }} saved {{ Str::plural('address', $addresses->count()) }}</p>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            @foreach($addresses as $address)
                                <div class="bg-white rounded-xl border {{ $address->is_default ? 'border-primary-200 ring-1 ring-primary-100' : 'border-neutral-100' }} p-5 relative group hover:shadow-sm transition-all">
                                    ...
                                    <p class="text-[16px] text-neutral-600 mt-0.5">{{ $address->phone }}</p>

                                    <!-- Address -->
                                    <p class="text-[16px] text-neutral-600 mt-2 leading-relaxed">
                                    ...
                                    <div class="flex items-center gap-3 mt-4 pt-3.5 border-t border-neutral-100">
                                        <a href="{{ route('account.addresses.edit', $address) }}"
                                           class="inline-flex items-center gap-1 text-[16px] font-medium text-primary-600 hover:text-primary-700 transition-colors">
                                        ...
                                        @if(!$address->is_default)
                                            <form action="{{ route('account.addresses.update', $address) }}" method="POST" class="inline">
                                                ...
                                                <button type="submit"
                                                        class="inline-flex items-center gap-1 text-[16px] font-medium text-neutral-600 hover:text-primary-600 transition-colors">
                                                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        <form action="{{ route('account.addresses.destroy', $address) }}" method="POST"
                                            ...
                                            @method('DELETE')
                                            <button type="submit"
                                                    class="inline-flex items-center gap-1 text-[16px] font-medium text-neutral-600 hover:text-error-600 transition-colors">
                                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @else
                        <div class="bg-white rounded-xl border border-neutral-100 p-12 text-center">
                            ...
                            <p class="text-[16px] text-neutral-600 mb-5">Add an address to make checkout faster</p>
                            <a href="{{ route('account.addresses.create') }}"
