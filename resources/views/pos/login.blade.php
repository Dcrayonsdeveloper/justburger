    ...
    <div class="flex items-center justify-center h-full" style="background: linear-gradient(135deg, #F8FAFC 0%, #E2E8F0 100%);">
        <div class="pos-card p-8 w-full max-w-md pos-fade-in" style="box-shadow: 0 8px 32px rgba(0,0,0,0.08);">
            ...
            <div class="text-center mb-6">
                ...
                @if($siteLogo)
                    <img src="{{ asset('storage/' . $siteLogo) }}" alt="{{ \App\Models\Setting::get('site_name', config('app.name')) }}" class="h-10 mx-auto mb-2">
                @else
                    <div class="text-2xl font-bold" style="color: var(--pos-primary);">{{ \App\Models\Setting::get('site_name', config('app.name')) }}</div>
                @endif
