...
<body>
    {{-- Store Header --}}
    <div class="center">
        <div class="bold" style="font-size: 16px;">{{ $sale->store->name ?? \App\Models\Setting::get('site_name', config('app.name')) }}</div>
        @if($sale->store?->address)
        <div style="font-size: 10px; margin-top: 2px;">{{ $sale->store->address }}</div>
        @endif
        @if($sale->store?->vat_number ?? $sale->store?->gst_number)
        <div style="font-size: 10px;">VAT No: {{ $sale->store->vat_number ?? $sale->store->gst_number }}</div>
        @endif
    </div>
    ...
    <div class="item-row">
        ...
        @if($item->tax_rate)
        <div class="item-detail">VAT: {{ $item->tax_rate }}%</div>
        @endif
    </div>
    @endforeach
    ...
    @endif
    @if($sale->tax > 0)
    <div class="row"><span>VAT</span><span>£{{ number_format($sale->tax, 2) }}</span></div>
    @endif
    ...
    {{-- VAT Summary --}}
    @php
        $vatItems = $sale->items->where('tax_rate', '>', 0);
    @endphp
    @if($vatItems->count() > 0)
    <div class="divider"></div>
    <div class="center bold" style="font-size: 10px; margin-bottom: 3px;">VAT BREAKUP</div>
