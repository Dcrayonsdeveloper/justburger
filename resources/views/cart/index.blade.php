<x-layouts.app>
    <x-slot name="title">Your Order — {{ \App\Models\Setting::get('site_name', config('app.name')) }}</x-slot>
        ...
        <div style="max-width:1100px; margin:0 auto; padding:1.5rem 1.25rem 3.5rem;">
                            ...
                            <template x-for="item in items" :key="item.id">
                                <div class="cart-item">
                                    ...
                                    <a :href="item.product_url" class="cart-item-img" style="text-decoration:none;">
                                        <img :src="item.image || '{{ asset('images/products/justburgers/cheeseburger.jpg') }}'"
                                             :alt="item.name"
                                             onerror="this.src='{{ asset('images/products/justburgers/cheeseburger.jpg') }}'">
                                    </a>
            ...
            <div style="margin-top:3rem;border-top:1px solid rgba(0,0,0,.08);padding-top:2rem;">
                ...
                <div style="display:flex;gap:.85rem;overflow-x:auto;padding-bottom:.75rem;-webkit-overflow-scrolling:touch;" class="scrollbar-hide">
                    ...
                    <div class="rec-card">
                        <a href="{{ $rec['url'] }}" style="display:block;aspect-ratio:1;overflow:hidden;">
                            <img src="{{ $rec['image'] ?: asset('images/products/justburgers/cheeseburger.jpg') }}"
                                 alt="{{ $rec['name'] }}"
                        ...
                        <div style="padding:.7rem .8rem;">
                            <a href="{{ $rec['url'] }}" style="text-decoration:none;">
                                <p style="font-size:.8rem;font-weight:700;color:#111111;line-height:1.3;" class="line-clamp-2">{{ $rec['name'] }}</p>
                            </a>
                            <p style="font-size:.9rem;font-weight:800;color:#C8102E;margin:.3rem 0 .6rem;">@price($rec['price'])</p>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            if (typeof gtag !== 'undefined') {
                gtag('event', 'view_cart', { currency:'GBP', value:{{ (float) $cart->total }}, items:{!! json_encode($ga4CartItems, JSON_UNESCAPED_UNICODE) !!} });
            }
        });
    </script>
    @endif
