...
<img src="{{ url('/images/logo.svg') }}" alt="{{ \App\Models\Setting::get('site_name', config('app.name')) }}" style="height:32px;">
</td></tr>
...
<p style="font-size:14px;color:#333;margin:0 0 16px;">Hi <strong>{{ $order->user->first_name ?? $order->guest_name ?? 'there' }}</strong>, thanks for ordering with {{ \App\Models\Setting::get('site_name', config('app.name')) }}!</p>
...
@endif
<tr><td style="padding:4px 14px;font-size:13px;color:#555;">Delivery</td><td style="padding:4px 14px;text-align:right;font-size:13px;color:{{ $order->shipping_cost > 0 ? '#333' : '#067D62' }};">{{ $order->shipping_cost > 0 ? format_price($order->shipping_cost) : 'FREE' }}</td></tr>
...
<p style="font-size:11px;color:#999;text-align:center;margin:16px 0 0;">Need help? Reply to this email or WhatsApp us at {{ \App\Models\Setting::get('site_mobile', '07368 998 035') }}</p>

</td></tr>

<tr><td style="background:#1b454a;padding:14px 20px;text-align:center;">
<a href="{{ url('/') }}" style="color:#F8931D;font-size:12px;text-decoration:none;margin:0 8px;">Menu</a>
...
<p style="color:#7a9a9e;font-size:10px;margin:8px 0 0;">&copy; {{ date('Y') }} {{ \App\Models\Setting::get('site_name', config('app.name')) }}. All rights reserved.</p>
</td></tr>
