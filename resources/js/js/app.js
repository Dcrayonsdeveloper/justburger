 ...
 */
window.formatCurrency = function(amount, currency = 'GBP') {
...
Alpine.store('cart', {
    ...
    async add(productId, quantity = 1, variantId = null) {
        ...
        try {
            ...
            await this.fetch();
            // Don't open drawer if the menu page sidebar is visible (desktop)
    ...
    async addWithToppings(productId, quantity = 1, variantId = null, toppingsAdded = [], toppingsRemoved = []) {
        this.isLoading = true;
        try {
            const response = await axios.post('/cart/add', {
                product_id: productId,
...
/**
 * Toppings modal store
 */
Alpine.store('toppingsModal', {
    isOpen: false,
    isLoading: false,
    productId: null,
    productName: '',
    ...
    async open(productId, quantity = 1, variantId = null) {
        ...
        try {
            ...
            // Fallback: add to cart without toppings
            this.close();
            await Alpine.store('cart').add(productId, quantity, variantId);
        } finally {
            this.isLoading = false;
    ...
    toggle(toppingId) {
        this.selected[toppingId] = !this.selected[toppingId];
    },

    isSelected(toppingId) {
    ...
    async confirm() {
        ...
        this.close();
        await Alpine.store('cart').addWithToppings(this.productId, this.quantity, this.variantId, added, removed);
    },
