...
class OtpService
{
    ...
    private function sendWhatsApp(string $phone, string $code, string $purpose): bool
    {
        ...
        try {
            $response = Http::withToken($token)->post("https://graph.facebook.com/v21.0/{$phoneId}/messages", [
                ...
                'type' => 'text',
                'text' => [
                    'body' => "Your " . \App\Models\Setting::get('site_name', config('app.name')) . " OTP is: *{$code}*\n\nUse this to {$purposeText}.\nValid for " . self::OTP_EXPIRY_MINUTES . " minutes.\n\nDo not share this code with anyone.",
                ],
            ]);
            ...
            Mail::send([], [], function ($m) use ($email, $code, $purpose) {
                ...
                $m->to($email)
                  ...
                  ->html("<div style='font-family:sans-serif;max-width:400px;margin:0 auto;padding:20px;'>
                    ...
                    <p style='font-size:12px;color:#999;margin-top:20px;'>— Team " . \App\Models\Setting::get('site_name', config('app.name')) . "</p>
                  </div>");
            });
    ...
    private function cleanPhone(string $phone): string
    {
        ...
        if (!str_starts_with($clean, '44') && strlen($clean) === 11 && str_starts_with($clean, '0')) {
