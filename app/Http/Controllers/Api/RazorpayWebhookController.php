...
class RazorpayWebhookController extends Controller
{
    ...
    private function sendWhatsAppPaymentNotification(
        ...
        $message = match ($status) {
            'authorized', 'captured' => "Hi {$name}! ✅ Payment of £{$payment->amount} received for your {$siteName} order #{$order->order_number}. Thank you for your order! Track it at: " . url("/orders/{$order->id}"),
            'failed' => "Hi {$name}, ❌ Payment for your {$siteName} order #{$order->order_number} failed. Please retry at: " . url("/orders/{$order->id}") . " or contact us for help.",
            default => null,
        };
        ...
        // Clean phone number (remove non-digits, ensure UK country code)
